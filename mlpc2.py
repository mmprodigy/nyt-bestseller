...
for book in dataList:
	...
	newlist = []
	newlist.append(booklist[2] or 0) # page num 0
	newlist.append(booklist[10] or 0) # av rating 1
	newlist.append(booklist[11] or 0) # rating count2
	
	author = {"author = "+ (booklist[3] or "") : 1}
	year = {"pubyear = " + (booklist[4] or "") : 1}
...
for i in range(len(bVariables)):
	for j in range(0, 5):
		val = unicode(bVariables[i][j]).encode('ascii','ignore')
#		print("bVariables[i][j] :" , unicode(bVariables[i][j] ))

		if not val.isalnum(): val = 0
		bMatrix[i].append(float(val))##Add in continuous features
	for j in range(5,len(bVariables[i])):
		for key in bVariables[i][j]:
			if key not in uniquenessCheck:
				uniquenessCheck.add(key)
				bMatrix[i].append(bVariables[i][j][key])
#				print(bVariables[i][j][key])
				for k in range(0, i):
					bMatrix[k].append(0)
...
print("length of bMatrix is: ", len(bMatrix))
for row in bMatrix:
	print len(row)
...
trainingB, testingB, trainingP, tesingP = train_test_split(bMatrix, pVariables, test_size = 0.33)
